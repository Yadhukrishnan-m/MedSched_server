// Doctor Controller
import { Doctor, Otp } from "../models/index.js";
...
const otpRecord = await Otp.findOne({
  ...
  order: [["createdAt", "DESC"]], // get the most recent one
    ...
    res.json({ message: "Registration successful", user: newDoctor });
  } catch (error) {
    console.log(error);
...
export const login = async (req, res) => {
  try {
    ...
    const user = await Doctor.findOne({ where: { email:email.trim().toLowerCase() } });
    console.log(user)
    if (!user) {
    ...
    // Generate tokens
    const accessToken = generateAccessToken(user.id, "doctor");
    const refreshToken = generateRefreshToken(user.id, "doctor");
...
export const refreshToken = async (req, res) => {
  try {
    ...
    const user = await Doctor.findByPk(decoded.userId);
    if (!user) {
      return res.status(401).json({ 
    ...
    const newAccessToken = generateAccessToken(user.id, 'doctor');
